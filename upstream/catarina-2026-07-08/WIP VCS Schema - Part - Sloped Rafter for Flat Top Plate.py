...
def _safe_new_doc(name, clear_existing=False):
    """Create or reuse a FreeCAD document without App.getDocument throwing."""
...
def compile_single_roof_rafter(schema=single_roof_rafter_schema):
    doc = _safe_new_doc(
        schema["document_name"],
        clear_existing=schema.get("clear_existing_document", False)
    )

    lumber_size = schema["lumber_size"]
    actual = schema["actual_size_in"][lumber_size]
    rafter_thickness = actual["thickness"]  # X direction
    rafter_depth = actual["depth"]
...
compile_single_roof_rafter(single_roof_rafter_schema)
