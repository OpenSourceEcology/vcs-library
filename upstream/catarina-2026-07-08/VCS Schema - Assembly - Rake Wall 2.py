...
def _new_doc(name):
    """
    Create a fresh FreeCAD document safely.
    App.getDocument(name) throws if the document does not exist, so use
    App.listDocuments() instead.
    """
...
def compile_rake_wall_assembly(schema):
    validate_schema(schema)

    doc = _new_doc(schema["name"])
...
compile_rake_wall_assembly(rake_wall_assembly_schema)
